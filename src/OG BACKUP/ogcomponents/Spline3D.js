    ...
    return lines.map((line, lineIndex) => {
      ...
      return (
        <div key={lineIndex} style={{ 
          ...
          {isVisible ? visibleLines[lineKey] : ''}
        </div>
      );
    });
  };
...
Please be aware this is very much in testing stages.
...
🎯 Use Tab key for auto-completion.` 
    ...
    // Add typewriter effect to new outputs (skip the command line, start from last output)
    // Add typewriter effect to new outputs (skip the command line, start from last output)
const lastOutputIndex = newOutput.length - 1;
if (lastOutputIndex >= 0 && newOutput[lastOutputIndex].type !== 'command') {
  ...
  return (
    ...
    >
      {/* <button 
        ...
        style={{
          backgroundColor: 'rgba(120, 80, 80, 0.8)', // Keep close button visible
          border: '1px solid rgba(150, 100, 100, 0.6)',
          color: '#d0d0d0'
        }}
      >
        ✕
